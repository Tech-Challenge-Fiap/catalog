...
def test_get_product_by_id_success(client, product_data):
    with patch(
        ...
        "system.application.usecase.products_usecase.GetProductByIDUseCase.execute"
    ) as mock_use_case:
        mock_use_case.return_value = GetProductByIDResponse(product_data)
        response = client.get(f"/get_product/{product_id}")
